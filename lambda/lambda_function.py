...
def lambda_handler(event, context):
    ...
    if http_method == 'POST':
        ...
        return {
            'statusCode': 500,
            'body': json.dumps("Server Inetrnal Error: Unknown Error Occured")
        }
...
def update_device(event):
    ...
    device_id = event['queryStringParameters']['device_id']
    updated_device_data = json.loads(event['body'])
    try:
        s3.put_object(
        Bucket=BUCKET_NAME,
        Key=str(device_id),
        Body=json.dumps(updated_device_data)
        )
        return {
            'statusCode': 200,
            'body': json.dumps("Device updated Successfully")
        }
    except s3.exceptions.NoSuchKey as e:
        print(f"{device_id} doesn't exist")
        return {
